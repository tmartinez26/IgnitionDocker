...
set_container_names() {
	mapfile -t temp_containers < <(docker-compose ps -q | xargs docker inspect --format '{{ index .Config.Labels "com.docker.compose.service" }} {{ .Name }} {{ .Config.Image }}' | grep -E 'ignition-docker|ignition' | sed 's/\///g' | awk '{if ($3 ~ /ignition-docker|ignition/) print $1, $2}')
